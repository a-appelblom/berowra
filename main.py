...
def index():
    print(request.args)
    res = collectionsDB.fetch(
        {"titleCaps?contains": request.args.get("search").upper()}
        if request.args.get("search") != None
        and request.args.get("search").replace(" ", "") != ""
        else {},
        # limit=1,
        # buffer=50000,
    )
...
def files():
    if request.method == "POST":
        ...
        if file:
            filename = secure_filename(file.filename)
            res = drive.put(
                str(datetime.datetime.now(utc)).replace(" ", "") + filename, file
            )
            items = drive.list()
            print(items)
...
def api():
    ...
    return render_template(
        "api.html",
        collections=items,
        exampleKey=contentItems[0]["key"] if len(contentItems) > 0 else None,
    )
...
def collectionEdit(id):
    ...
    return render_template(
        ...
        collectionId=id,
        collectionIdDisplay="(" + id + ")",
        collectionItems=data["templateItems"],
    )
...
def content(id):
    if request.method == "POST":
        ...
        for x in enumerate(formData):
            if x[1][0] == "content_title":
                ...
                if x[1][1] == "on":
                    content[int(x[1][0].split("-file-checkbox-")[0])]["value"].append(
                        x[1][0].split("-file-checkbox-")[1]
                    )
    ...
    if getContentData["content"] == {}:
        ...
        for x in getCollectionData["templateItems"]:
            ...
            if "title" in x and str(x["id"]) not in getContent:
                ...
                x["value"] = (
                    getContent[str(x["id"])]["value"]
                    if "value" in getContent[str(x["id"])]
                    else ""
                )
    ...
    return render_template(
        ...
        published="checked" if getContentData["published"] == True else "",
        contentId=id,
    )
...
def apiCollection(id):
    ...
    def formatItem(n):
        return (
            {"key": n["key"], "lastUpdated": n["lastUpdated"], "title": n["title"]}
...
def apiCollections():
    ...
    def formatItem(n):
        return (
            {"key": n["key"], "lastUpdated": n["lastUpdated"], "title": n["title"]}
            if includeTemplate == False
            else {
                "key": n["key"],
                "lastUpdated": n["lastUpdated"],
                "title": n["title"],
                "templateItems": n["templateItems"],
            }
        )
